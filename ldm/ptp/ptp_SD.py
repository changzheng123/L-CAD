...
from ldm.ptp import seq_aligner
import sys
...
def show_cross_attention(tokenizer, prompts, n, attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0, dir_path=None, name=None):
    ...
    # sys.exit()
    images = []
    words_num = 77 if len(tokens)>77 else len(tokens)
    for i in range(words_num):
        ...
        images.append(image)
    ptp_utils.view_images(np.stack(images, axis=0),n,name=name[0].split('.')[0]+'_'+prompts[0][0:150],dir_path=dir_path)
...
def run_and_display(prompts, controller, latent=None, run_baseline=False, generator=None):
    ...
    ptp_utils.view_images(images,name=prompts[0])
    return images, x_t
