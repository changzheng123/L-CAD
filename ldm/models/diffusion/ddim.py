...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      ucg_schedule=None,use_attn_guidance=False, sam_mask=None):
        ...
        for i, step in enumerate(iterator):
            ...
            if not use_attn_guidance:
                ...
                outs = self.p_sample_ddim_with_attn_gdc(img, cond, ts, sam_mask, index=index, use_original_steps=ddim_use_original_steps,
                                        quantize_denoised=quantize_denoised, temperature=temperature,
                                        noise_dropout=noise_dropout, score_corrector=score_corrector,
                                        corrector_kwargs=corrector_kwargs,
                                        unconditional_guidance_scale=unconditional_guidance_scale,
...
class DDIMSampler_withsam(object):
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                      unconditional_guidance_scale=1., unconditional_conditioning=None, dynamic_threshold=None,
                      ucg_schedule=None,use_attn_guidance=False, sam_mask=None,split_id=None,tokens=None):
        device = self.model.betas.device
        b = shape[0]
        if x_T is None:
            img = torch.randn(shape, device=device)
        ...
        for i, step in enumerate(iterator):
            ...
            if not use_attn_guidance:
                ...
                outs = self.p_sample_ddim_with_attn_gdc(img, cond, ts, sam_mask, split_id, index=index,tokens=tokens, use_original_steps=ddim_use_original_steps,
                                        quantize_denoised=quantize_denoised, temperature=temperature,
                                        noise_dropout=noise_dropout, score_corrector=score_corrector,
                                        corrector_kwargs=corrector_kwargs,
                                        unconditional_guidance_scale=unconditional_guidance_scale,
